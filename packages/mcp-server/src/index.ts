...
import { XunjiStore, defaultDatabasePath } from './store'
...
function createServer() {
  const store = new XunjiStore()
  const server = new McpServer(
    { name: 'xunji', version: '0.2.0' },
    { instructions: '只读检索寻迹中已发布的知识。草稿、任务和供应商配置不可访问。' },
  )
  const readOnly = { readOnlyHint: true }

  server.registerTool('xunji_search', {
  ...
  server.registerTool('xunji_get_card', {
  ...
  server.registerTool('xunji_list_facets', {
  ...
  server.registerTool('xunji_get_source', {
...
try {
  ...
  console.error(`xunji-mcp: read-only database ${defaultDatabasePath()}`)
  process.on('SIGINT', () => void handle.close())
} catch (error) {
  console.error(`xunji-mcp: ${error instanceof Error ? error.message : error}`)
  process.exit(1)
}
