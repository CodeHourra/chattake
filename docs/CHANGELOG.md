...
- `DistillerConfig.max_concurrent_analyses` 支持 1–8 的动态并发上限，默认 2；分析任务共享读锁，同步任务继续持有写锁。
- 每个运行中的分析条目使用独立 Sidecar；RPC 响应按请求 ID 分发，取消和超时只影响对应条目。
- 在事务内拒绝同批次重复会话及已排队/运行会话；取消后正确完成条目计数并允许重新分析。

### 桌面与发布

- 顶栏进度中心支持逐项清除终态任务，补齐等待、单项处理和并行处理状态及无障碍语义。
- 分析完成后按最新 `cardId` 刷新详情并替换 URL；启动失败不再留下错误的分析中状态。
- `softprops/action-gh-release` 升级到 `v3.0.3`，使用 Node.js 24 运行时。
