...
- **六类对话来源**：Claude Code、Cursor、Codex、CodeBuddy、OMP、Pi；增量扫描不自动消耗 Token。
...
2. **同步会话**：点击顶栏「同步」。右下角任务中心会展示扫描、解析、写入进度和失败文件；取消会在当前文件处理完成后生效。
3. **配置分析 Provider**：打开「设置 → AI 配置」，新增 API 或 CLI 配置并设为当前。API 支持刷新模型列表或手填完整模型 ID；CLI 的「检测命令」只检查可执行性，不调用模型。
